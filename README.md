...
Les horaires sont configurables par serveur dans `/config` : choisissez **Horaire du planning (cron hebdomadaire)** ou **Horaire des devoirs (cron hebdomadaire)**, puis saisissez `minute heure * * jour` dans `value`. Par exemple, `30 17 * * 5` signifie vendredi à 17 h 30, heure de Paris (0 = dimanche, 6 = samedi). `/config action:get` et `/config action:list` affichent aussi l'horaire en clair. Le changement prend effet immédiatement et reste enregistré après redémarrage. Les valeurs par défaut sont `0 18 * * 0` pour le planning et `0 10 * * 6` pour les devoirs. Le rappel des devoirs doit toujours être activé via `/rappeldevoir configurer` ; le planning doit avoir un salon et une connexion MyGES configurés.
